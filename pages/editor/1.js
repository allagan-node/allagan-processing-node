      ...
      ReadBytesFromFile(indexFiles[i]).then(b => {
        if (new TextDecoder().decode(b.slice(0, 6)) !== "SqPack") {
          this.state.errors.push(
            indexFiles[i].name + " 파일 포맷이 잘못되었습니다."
          );
        } else {
  ...
  validateDatFiles(i, datToValidate, files) {
    ...
    this.setState(this.state, () => {
      ReadBytesFromFile(datToValidate[i].slice(0, 6)).then(b => {
        const fileName = datToValidate[i].name.substring(
          0,
          datToValidate[i].name.indexOf(".")
        );
        const datNum = datToValidate[i].name.substring(
          datToValidate[i].name.length - 1,
          datToValidate[i].name.length
        );
        ...
        if (
          ...
          this.state.errors.push(
            datToValidate[i].name + " 파일 포맷이 잘못되었습니다."
          );
        } else {
